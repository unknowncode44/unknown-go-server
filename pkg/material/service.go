...
func (s *service) Create(material *entities.Material) (*entities.Material, error) {
	...
	if !material.Group.IsValid() {
		return nil, errors.New("material group must be BDC, BDU or CUSTODIA")
	}
...
func (s *service) FindByERPCode(erpCode string) ([]entities.Material, error) {
	return s.repo.FindByERPCode(erpCode)
}
